              ...
              <!-- <button data-toggle="modal" data-target="#modal-tambah" type="button" class="btn btn-md btn-primary btn-absolute">Tambah</button> -->
                <table id="tabledata" class="table  table-striped">
                  ...
                  <tr>
                    ...
                    <td width="20%">{{$key->kategori==0 ? "Rekening" : "E-Wallet"}}</td>
                    <td>{{$key->partner}}</td>
                    <td>{{$key->nama}}</td>
                    <td>{{$key->no}}</td>
              ...
              <!-- <div class="card-body"> -->
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="kategori_{{$key->id}}" name="kategori[]">
                      <option value="0" {{$key->kategori == 0 ? "selected" : ""}}>Rekening</option>
                      <option value="1" {{$key->kategori == 1 ? "selected" : ""}}>E-Wallet</option>
                    </select>
                </div>
                <div class="form-group">
                    <label for="partner_{{$key->id}}">Partner (NAMA BANK/ NAMA E-WALLET)<span class="bintang">*</span></label>
                    <input type="text" class="form-control" id="partner_{{$key->id}}" name="partner[]" placeholder="Partner" value="{{$key->partner}}">
                </div>   
                <div class="form-group">
                    <label for="nama_{{$key->id}}">Nama Pemilik<span class="bintang">*</span></label>
                    <input type="text" class="form-control" id="nama_{{$key->id}}" name="nama[]" placeholder="Nama Pemilik" value="{{$key->nama}}">
                </div>
                <div class="form-group">
                    <label for="no_{{$key->id}}">No.Rekening<span class="bintang">*</span></label>
                    <input type="text" class="form-control" id="no_{{$key->id}}" name="no[]" placeholder="No.Rekening" value="{{$key->no}}">
                </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <form method="post" id="formHapus_{{$key->id}}" class="form-horizontal">
            @csrf
            <div class="modal-body">
                <h6> Apakah anda ingin menghapus data No.Rekening/E-Wallet {{$key->nama}}?</h6>
            </div>
            <div class="modal-footer justify-content-between">
                <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
...
<script>
  $(document).ready(function(){
    ...
    $(document).on('click', '.btn-hapus', function (e) {
        ...
        var url = "{{ url('/hapusmasterrekening') }}/"+idform;
        $.ajaxSetup({
            headers: {
    ...
    $(document).on('click', '.btn-submit-data', function (e) {
        ...
        $('#formData_'+idform).validate({
          ...
          submitHandler: function () {
            ...
            var url = "{{ url('/updatemasterrekening') }}/"+idform;
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
    ...
    $('#_formData').validate({
          ...
          submitHandler: function () {
            ...
            var url = "{{ url('storemasterrekening') }}";
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
