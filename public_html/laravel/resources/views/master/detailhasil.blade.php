@extends('layouts.SkydashAdmin')
<!-- partial -->
@section('content')
@php
  $cekbenar = App\Models\UPaketSoalDtl::where('fk_u_paket_soal_mst',$upaketsoalmst->id)->where('cek_benar',1)->get();
  $ceksalah = App\Models\UPaketSoalDtl::where('fk_u_paket_soal_mst',$upaketsoalmst->id)->where('cek_benar',0)->get();
@endphp
<div class="content-wrapper">
<h3 class="font-weight-bold">Detail hasil ujian {{$user->name}} ({{Carbon\Carbon::parse($upaketsoalmst->mulai)->translatedFormat('l, d F Y , H:i:s')}})</h3>
<br>
  <div class="row">
    <div class="col-md-12">
      <div class="row">
        <div class="col-xl-9 col-md-9 col-sm-9 col-xs-9">
          <h4 class="font-weight-bold">{{$upaketsoalmst->judul}}</h4>
        ...
        <div class="col-xl-3 col-md-3 col-sm-3 col-xs-3">
            <button type="button" class="btn btn-outline-success btn-sm _btn_benar_salah">
              <h6>Jawaban Benar : {{count($cekbenar)}}</h6>
            </button>
            <button type="button" class="btn btn-outline-danger btn-sm _btn_benar_salah">
  ...
  <div class="row">
    ...
    <div class="_soal_content tab-content" id="pills-tabContent">
      ...
      <div class="tab-pane fade {{$key->no_soal==1 ? 'show active' : ''}}" id="pills-{{$key->id}}" role="tabpanel">
          ...
          <h5><b>Jawaban User</b> : {{$key->jawaban_user ? $key->jawaban_user : '-'}} ({{$key->cek_benar==1 ? "Benar" : "Salah"}})</h5>
          <br>
